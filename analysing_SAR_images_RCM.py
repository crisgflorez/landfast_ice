...
csv_path = "/dmidata/users/cgf/files/overlap_results_rcm.csv"
error_csv_path = "/dmidata/users/cgf/files/sar_files_rcm_with_errors.csv"
missing_csv = "/dmidata/users/cgf/files/missing_files_rcm.csv"
...
for name in os.listdir(base_path):
    ...
    # =========================
    # Check if there are RCM files in the folder 
    # =========================

    rcm_files = glob.glob(os.path.join(path, "RCM*.zip"))
    # if no RCM files -> skip folder
    if len(rcm_files) == 0:
        print(f"No RCM files in folder: {name}")
        continue
...
#    {'name': ['nc_footprint', 'gcp_polygon']},
#    geometry=[nc_footprint, gcp_polygon],
#    crs="EPSG:3411"
#)
...
# Mask for selected poly_ids
#mask = np.isin(ice_array, poly_ids)
# Extract coordinates
#lons_sel = X[mask]
#lats_sel = Y[mask]

#plt.scatter(lons_sel, lats_sel, s=1, color="blue", alpha=0.5, label="Points (last pid)")
# =========================
# 2. INDIVIDUAL POLYGONS
# =========================
#for poly in polygons:
#    x, y = poly.exterior.xy
#    plt.plot(x, y, color="orange", linewidth=5,alpha=1)



#plt.figure(figsize=(8, 8))
#plt.scatter(lons_sel, lats_sel, s=1)
#plt.title("Pixels belonging to selected poly_ids")
#plt.xlabel("Longitude (EPSG:3411)")
#plt.ylabel("Latitude (EPSG:3411)")
#plt.show()

#vals = ds.polygon_icechart.values
# IDs únicos sin NaN
#unique_vals = np.unique(vals[~np.isnan(vals)])
#cmap = plt.cm.get_cmap("tab20", len(unique_vals)).copy()
# NaN transparente
#cmap.set_bad(alpha=0)
# boundaries entre IDs
#bounds = np.arange(unique_vals.min() - 0.5,
#                   unique_vals.max() + 1.5,
#                   1)

#norm = BoundaryNorm(bounds, cmap.N)
# masked array para NaN
...
#mask = np.isin(vals_masked, poly_ids)
#plt.figure(figsize=(10,10))
#plt.imshow(mask)
#plt.colorbar()
#plt.title("Fast ice poly_ids")
#plt.show()
